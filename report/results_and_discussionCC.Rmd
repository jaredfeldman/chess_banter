...
exp_data_comb <- fread('../data/exp_data_combined.csv')
exp_data_comb <- exp_data_comb[maia_name %in% c('ucb_123','chess4gerry', 'sandman353')]
```


```{r feature engineer, include=FALSE}
# one-hot encode maia win
exp_data_comb[ , maia_win := ifelse(winner == 'maia', 1, 0)]

# one-hot encode maia checkmate
exp_data_comb[ , maia_checkmate := ifelse(winner == 'maia' & status == 'mate', 1, 0)]

# one-hot encode opp checkmate
exp_data_comb[ , opp_checkmate := ifelse(winner == 'opp' & status == 'mate', 1, 0)]

# one-hot encode opp resign
exp_data_comb[ , opp_resign := ifelse(winner == 'maia' & status == 'resign', 1, 0)]

# one-hot encode draw
exp_data_comb[ , draw := ifelse(status == 'draw', 1, 0)]

# one-hot encode treatment of ChatGPT
exp_data_comb[ , treat := ifelse(chat_type == 'ChatGPT', 1, 0)]
...
exp_data_gpt_c_c <- exp_data_comb[chat_type %in% c("Control", "Control GPT") , ]
#exp_data_c_gpt <- exp_data_c_gpt[complete.cases(exp_data_c_gpt$respond), ]

#exp_data_c_gpt[, opp_rating := opp_rating / 1500]

comb_gpt_itt_win           <- exp_data_gpt_c_c[ , lm(maia_win ~  num_moves + opp_rating + chat_type)]
comb_gpt_itt_maia_mate     <- exp_data_gpt_c_c[ , lm(maia_checkmate ~  num_moves + opp_rating + chat_type)]
comb_gpt_itt_opp_mate      <- exp_data_gpt_c_c[ , lm(opp_checkmate ~  num_moves + opp_rating + chat_type)]
comb_gpt_itt_opp_resign    <- exp_data_gpt_c_c[ , lm(opp_resign ~ num_moves + opp_rating  + chat_type)]
comb_gpt_itt_draw          <- exp_data_gpt_c_c[ , lm(draw ~ num_moves + opp_rating  + chat_type)]
comb_gpt_itt_opp_acc       <- exp_data_gpt_c_c[ , lm(opp_acc ~ num_moves + opp_rating  + chat_type)]
comb_gpt_itt_opp_blunders  <- exp_data_gpt_c_c[ , lm(opp_blunders ~ num_moves + opp_rating  + chat_type)]
comb_gpt_itt_opp_mistakes  <- exp_data_gpt_c_c[ , lm(opp_mistakes ~ num_moves + opp_rating + chat_type)]
comb_gpt_itt_opp_acpl      <- exp_data_gpt_c_c[ , lm(opp_acpl ~ num_moves + opp_rating + chat_type)]
comb_gpt_itt_opp_move_time <- exp_data_gpt_c_c[ , lm(opp_avg_move_time ~ num_moves + opp_rating + chat_type)]

```


```{r, out.extra='angle=90', include=TRUE, results='asis', echo=FALSE}
comb_gpt_models_itt <- list(comb_gpt_itt_win,
          comb_gpt_itt_maia_mate,
          comb_gpt_itt_opp_mate,
          comb_gpt_itt_opp_resign,
          comb_gpt_itt_draw,
          comb_gpt_itt_opp_acc,
          comb_gpt_itt_opp_blunders,
          comb_gpt_itt_opp_mistakes,
          comb_gpt_itt_opp_acpl,
          comb_gpt_itt_opp_move_time
          )
...
stargazer(comb_gpt_models_itt,
          type = 'text',
          header = FALSE)
```

```{r}
model_rating <- exp_data_gpt_c_c[ , lm(opp_rating ~ chat_type)]
