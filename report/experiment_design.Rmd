...
In this project, we created a chess bot by harnessing the power of the Maia neural network ("Maia") as detailed in "The human side of AI for chess" by Microsoft Research\footnote{Mcllroy-Young et al., 2020 - The human side of AI for chess.}. Our approach involved conducting two experiments. The first served as a preliminary exploration, identifying potential outcome variables that would be crucial in our subsequent experiment. For the second experiment, we integrated the OpenAI API ("ChatGPT") to facilitate more dynamic conversations.
