...
Our main outcome variable of interest was Accuracy, as described in section 3.1. Figure 2 shows the distribution of Accuracy ITT across treatment groups for each experiment, while Tables 1 and 2 show the ITT for Accuracy and five other outcomes of interest. The tables containing all ten outcomes can be found in Appendix C.

As seen below, We found no statistical significance at the .05 level in either experiment as seen in Tables 1 and 2.
...
While the $ITT$ allows us to get a glimpse into potential treatment effects, it does not consider compliers and non-compliers. In Figure 3, we can see not everyone received their targeted dose of treatment, which we now take into account in calculating the Complier Average Causal Effect ($CACE$).
...
\end{quote}
```
where $ITT_d$ is the proportion of compliers in the treatment group (the "take-up rate"). In our case, we can see in Figure 3 that the take-up rates are approximately 30% and 33% for the preliminary and main experiments, respectively.
