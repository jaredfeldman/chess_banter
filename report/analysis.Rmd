...
\end{quote}
```
where z signifies the assignment of the subject. Below shows the ITT across six of our ten 10 different outcome variables for each experiment. The table containing all ten outcomes can be found in the Appendix.  We found no statistical significance at the .05 level in either experiment as seen in tables 1 and 2.  
...
get_stargazer(exp_data_c_t, "Preliminary Experiment")
```

```{r, results='asis', echo=FALSE, warning=FALSE, message=FALSE, include=TRUE}
get_stargazer(exp_data_main, "Main Experiment")
```
...
While the $ITT$ allows us to get a glimpse into potential treatment effects, it does not consider compliers and non-compliers. In Figure 2, we can see not everyone received their targeted dose of treatment, which we now take into account in calculating the Complier Average Causal Effect ($CACE$).
...
\end{quote}
```
where $ITT_d$ is the proportion of compliers in the treatment group (the "take-up rate"). In our case, we can see in Figure 2 that the take-up rates are approximately 30% and 33% for the preliminary and main experiments, respectively.

Now we will run the same regression analysis, but this time we will adjust our effects by predicting outcomes based on the CACE methods described above. In Table 3, for our preliminary experiment we see a slight effect on Opponent resignations and small uptick in Opponent Mistakes with a $p$ value < .1. In Table 4 for our Main Experiment, we see no statistical significance for outcomes except on Opponent Average Move Time being up more than 5.8 seconds over those in the control group. 
